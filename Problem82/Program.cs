namespace Problem82
{
    ...
    internal class MatrixPuzzle
    {
        ...
        public MatrixPuzzle(Int64[,] input)
        {
            ...
            for (Int32 i = 0; i < size; i++)
            {
                for (Int32 j = 0; j < size; j++)
                {
                    if (j == 0)
                    {
                        Int64 distance = matrix[i, 0];
                        distances[i, j] = distance;
                        unvisitedNodes.Enqueue((i, 0), distance);
                    }
                    else
                    {
                        distances[i, j] = null;
        ...
        Int64 GetWinningValue()
        {
            ...
            for (Int32 i = 0; i < size; i++)
            {
                if (distances[i, size - 1] == null)
                {
                    continue;
                }
                Int64 next = distances[i, size - 1].Value;
                if (minValue == null)
                {
                    minValue = next;
                }
